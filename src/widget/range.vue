<template>
  <div class="range" @mousemove="mouseMove($event)" @mouseup="mouseUp" @mouseleave="mouseUp">
    ...
    <div class="duration" ref="duration" @click.stop="setCurrentProgress($event)">
      ...
      <span class="ball" ref="ball" :style="progressBall" @mousedown="mouseDown" @touchstart="mouseDown"
            @touchmove="touchMove($event)" @touchend="touchEnd($event)"></span>
    </div>
  ...
  export default {
    ...
    watch: {
      volume: function (newvolume, oldvolume) {
        if (newvolume !== true) {
          let ballTrans = getComputedStyle(this.$refs.ball).transform
          let contentTrans = getComputedStyle(this.$refs.currentProgress).transform
          this.$refs.currentProgress.style.transform = contentTrans === 'none' ? ballTrans : ballTrans.concat('', contentTrans)
        }
      }
    },
    methods: {
