<template>
  <transition name="sliderUpHideRight">
    <div class="musicdetail" v-show="isShowMusicDetail">
      <div class="filterbg"
           :style="{backgroundImage : 'url(' + this.getCurrentMusic.img_url + ')', backgroundSize : 'cover', backgroundPosition : 'center center'}">
      </div>
      <div class="detailcontent">
          ...
          <div class="lrc-wrapper" ref="lrcwrapper" v-show="!showCD" @click.stop="isShowCD(true)">
            ...
            <div class="lrc" ref="lrc" :scroll-top.prop="scrollTop">
              <div class="div-lrc" ref="divlrc">
                <p v-show="getCurrentMusic" v-for="(lrc, index) in getCurrentMusic.lyric" :key="index"
                   :class="lrcIndex === index ? 'active musiclrc' : 'musiclrc'" :data-timeid="lrc.timeId">{{lrc === '' ?
                  '暂无歌词' : lrc.text}} </p>
              </div>
            </div>
          </div>
        ...
        <div class="content-footer">
          ...
          <div class="musicDetailCtrl">
            ...
            <i class="playPause" :class="isPlaying ? 'icon-pause-detail' : 'icon-playdetail' "
               @click.stop="playPause"></i>
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  .musicdetail
    ...
    .filterbg
      ...
      right: 0
      z-index: 16
      filter: url('../common/blur.svg') blur(30px)
    .detailcontent
      position: fixed
      height: 100%
      width: 100%
